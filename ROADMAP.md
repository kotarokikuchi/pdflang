...
| Comparison | 🟧 partial — text diff with LCS page alignment, pHash, SSIM, pixel diff, Delta-E; `pixelcompare` adds a rendered per-page diff with global-shift alignment, added/removed/recoloured classification and a three-pane viewer with a wipe; **no** typography/table/vector diff, no anchor alignment, no per-element moved-vs-changed semantics, no accept/reject/review triage |
