...
## 1.5 `if`: الاختيار بين شيئين

`if` **تعبير**، مثل كل ما يُنتج قيمة هنا: يعيد آخر تعبير في الفرع الذي جرى.
ولذلك يصلح قيمةً:

```pdfl
check "حدّ الحبر يتبع نوع الورق" {
  // vars.stock تأتي من ‎--var stock=coated
  const LIMIT = if vars.stock == "coated" { 300 } else { 260 }

  doc.pages.each { |page|
    assert page.tac <= LIMIT,
      "الصفحة #{page.number}: حبر #{page.tac}%، والحدّ #{LIMIT}%"
  }
}
```

ويصلح حارسًا حول الأوامر، بلا `else`:

```pdfl
check "لا تفحص الغلاف إلا إن وُجد" {
  if doc.page_count > 1 {
...
و`else if` تتسلسل بلا أقواس إضافية:
...
- **الفرع الذي لا يجري يعطي `null`.** فـ`if` بلا `else` وشرطه كاذب قيمته `null`
  — وهي قيمة كاذبة، فتصلح للاختبار مباشرة.
- **لكل فرع نطاقه الخاص**، كالكتلة والدالة. فالمتغير الذي يُنشأ داخل الفرع لا
  يبقى بعده. أما الإسناد إلى متغير موجود في الخارج فيظل يحدّث ذلك المتغير.
- **القوس `{` بعد الشرط يفتح المتن.** فإن انتهى الشرط نفسه بكتلة فضعه بين
  قوسين، وإلا قُرئ قوس المتن على أنه قوس تلك الكتلة:

```pdfl
// خطأ: يُؤخذ الـ { متنًا لـ if
...
> `if` لاختيار قيمة أو لحماية أمر — لا لاستبدال تحقّق أخفق بتحقّق صامت. فالتدقيق
> الذي يجب أن يُخفق يظل يُخفق؛ انظر [1.2](#12-طريقتان-للتدقيق).

---

## 1.6 الكتل: التكرار على كل عنصر
...
## 1.8 import: إعادة الاستعمال بين الملفات التعريفية
...
`--var الاسم=القيمة` في `pdfl run` و`pdfl test` و`pdfl watch` تصل إلى النص البرمجي
بصيغة `vars.الاسم`، وينقل `test` و`watch` القيمة نفسها إلى كل حالة أو ملف —
اسم عميل واحد للتشغيلة كلها، لا واحدًا لكل ملف.
ونصًّا دائمًا. وهي ما يمنع ملفًا تعريفيًا واحدًا من أن يصير خمس نسخ تكاد تتطابق:

```pdfl
check "العمل مطابق للطلب" {
  assert doc.title.contains(vars.order),
    "الملف يقول \"#{doc.title}\"، والطلب هو #{vars.order}"
}
```
...
والاسم الذي لم يُمرَّر **خطأ يسمّي الخيار الذي كان سيوفّره**، لا سلسلة فارغة: إذ
إن فحصًا يقارن بلا شيء كان سيمرّ، فيُبلغ عن ملف لم يتحقق منه أحد.

---

## 1.11 رسائل تنفع من يستلم الملف
...
| `the '{' here opens the body of the if` | شرط `if` ينتهي بكتلة | ضع الشرط بين قوسين |
