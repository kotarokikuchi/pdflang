...
[← سطر الأوامر](11-cli.md) · [الفهرس](README.md) · [التالي: سجل التغييرات →](13-changelog.md)
...
ومجلد للاطلاع، لمن عليه اعتماد إعادة الطبع:

```bash
pdfl pixelcompare approved/catalogue_v1.pdf received/catalogue_v2.pdf \
  --max-diff 0.05 --viewer proof/ --output-file pixels.json

zip -r proof.zip proof/    # ملف index.html وثلاث صور PNG لكل صفحة، لا غير
```

لا يحتاج المجلد إلى خادم ولا إلى شبكة: فمن يفتح `index.html` يرى الأصل والملف
الجديد وكليهما معًا وفوقهما الفروق ملوّنة، وهو يفتح على الصفحات المختلفة — ففي
كتالوج تغيّرت فيه صفحتان من تسعين، هاتان الصفحتان هما المقصود. وتحريك الفأرة
يمرّر الملف الجديد فوق القديم، وتُكبّر العجلة الألواح الثلاثة معًا ويحرّكها
السحب، فيُحسم خيط رفيع عند ثمانية أضعاف دون مغادرة الصفحة.
