...
`pdfl run`・`pdfl test`・`pdfl watch` の `--var 名前=値` は、スクリプトからは
`vars.名前` として、常に文字列として読めます。`test` と `watch` は同じ値をすべて
のケースやファイルに渡します——ファイルごとに1つではなく、実行全体で1つの
クライアント名、というように。1つのプロファイルがほとんど同じ5つのコピーに増えるのを防ぐのが
これです。
