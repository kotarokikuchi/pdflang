...
拡張子が `.json` のファイルは JSON として読みます。`load_dataset` でも
`lookup_value` でも同じです。受け付ける形は2つ。データセットが実際に書かれる
のがこの2つだからです。
...
オブジェクトの配列は、ヘッダー行1つ＋オブジェクトごとに1行になります。列の順序
はアルファベット順ではなく、**最初の**オブジェクトが書いた順です。つまり最初の
キーが、そのまま `lookup_value` の探すキーになります。
...
後続のオブジェクトでキーが欠けている場合は**空のセル**になり、行がずれることは
ありません。空欄はレポートで見えますが、ずれは見えないからです。数値は書かれた
ままの桁を保ち、`null` は空のセル——CSV の空フィールドと同じ意味です。

1つのファイルで2つの形を混ぜるとエラーになり、何行目かを示します。
