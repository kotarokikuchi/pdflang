...
13のコマンド：PDF を扱うもの5つ、スクリプトを扱うもの5つ、配布用が2つ、シェル用
が1つ。
...
`--viewer diff/` は、1ページにつき3つの PNG と `index.html` を1つ書き出します。
依存関係は一切ありません——CDN もバンドラもサーバも不要です。ファイルを開くか、
フォルダを zip にして、刷り直しを承認する人に送ってください。

同じ2つを見る方法が3つあります。

- **Wipe** — ページ上をドラッグして、新しいファイルを古いファイルの上に払う。
  何かが動いたかを見るのに一番速い方法です。
- **Flip** — ポインタを押している間だけ入れ替える。動いたものは飛び跳ね、
  動かなかったものはそのままです。
- **Fade** — 2つを混ぜる。色や太さの変化を判断するのに向きます。

どのモードでも差分はその場に着色され、色が種類を示します。
...
**Differences** の横のスライダーでその層を薄くでき、ビューアを離れずに下の
ページを確認できます。`←` `→` でページ送り、`space` で切り替え、`d` で層の
オン・オフ。

**Pages** は下のページ一覧を絞り込みます——**All**、または **Changed only**。
200ページのうち3ページだけが動いた文書で、残りの197ページを送っていくのが仕事の
遅い部分です。矢印キーはこの絞り込みに従い、隠れているページは飛ばして進みます。
変化のないページを見ている状態で *Changed only* に切り替えると、いちばん近い
変化したページへ移動します。どこも変わっていないときは、一覧を空にするのでは
なく、ボタンがその旨を示して無効のままになります。
