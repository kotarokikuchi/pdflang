...
3つのペインは同じ位置に同じ2本のバー——縦と横——を持ち、3つとも同時に動きます。
2本ともポインタについてきます——つかむものも狙うものもなく、交点は
マウスのある場所そのものです。2本が交わる点が、現れている領域の角です。

**Difference** のペインでは2本とも切れ目になります。新しいファイルは縦のバーの
右かつ横のバーの下に現れ、それ以外は元のファイルです。触らなければ横のバーは
一番上にあり、縦のバーはそれだけで高さいっぱいのワイプになります——探している
変化が列ではなく帯にあるときに、横のバーを下ろしてください。残る2つのペインでは、
2本はページの同じ列・同じ行を通る定規です。

どちらの位置もペインではなくページに対する割合なので、ページを切り替えても
ウィンドウの大きさを変えても保たれます。

ホイールで最大8倍までズームでき、3つのペインはポインタの下の点を中心に一緒に
拡大します——見ていたものがその場に留まります。縮小はページが収まる大きさで
止まります。それ以下に意味はありません。ペインはもともとページ全体を収める
大きさだからです。バーの太さはどのズームでも変わりません。**どちらのボタン**でも
押したまま動かすと、ページそのものを3つのペインまとめて、ズームに関わらず
ポインタと同じ速さで動かせます——拡大したあとに端へ行くための操作です。その間も
バーはポインタについてきます。
**Reset view** はズーム、移動、バーを最初の状態に戻します。戻すものが何もない
あいだ、このボタンは無効です。
...
別のページへ移ると、また全体表示から始まります。ズームや移動はそれを設定した
ページのものであり、持ち越すと次のページのたまたま何かある場所に着いてしまう
からです。どの経路で移っても同じです——一覧、矢印、ボタン。
