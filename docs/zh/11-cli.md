...
**Pages** 用来筛选底部的页码条：**All**，或者 **Changed only**。两百页的文档里
只有三页动过，翻过其余一百九十七页才是这件事真正费时的部分。方向键会跟随筛选——
跳过被隐藏的页面，而不会停在上面——在一个没有变化的页面上切到 *Changed only*，
会跳到最近的那个有变化的页面。什么都没变时，按钮会说明情况并保持禁用，而不是把
页码条筛成空的。
