...
三个面板带着同一条竖线，位置相同，在任意一个里拖动，三条一起移动。在
**Difference** 面板里这条线是切口：左边是原件，右边是新件。在另外两个面板里
它是一把尺，落在页面的同一列上——擦除切在哪里，在两份原件上都能直接找到，不必
凭眼睛估。位置是相对于页面而不是面板的百分比，所以换页和改变窗口大小都不会
丢失。
