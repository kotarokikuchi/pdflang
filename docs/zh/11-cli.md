...
三个面板带着同样的一对线——一竖一横——位置相同，并且三组一起移动。两条线都跟着指针走——没有要抓的东西，也没有要瞄的目标，交点就是鼠标所在的
位置。两条线相交的地方，就是被揭开区域的那个
角。
...
滚轮可以缩放，最多 8 倍，三个面板围绕指针下的那一点一起放大——你刚才在看的
地方仍然停在原处。缩小到整页适配为止：再往下没有意义，面板本来就是按装下
整页来定尺寸的。任何缩放下线条都保持原有的粗细。按住**任意一个鼠标键**拖动可以移动页面本身，三个
面板一起动，且无论缩放多少都与指针同速——放大之后要去某个角落，靠的就是它。
拖动期间线条仍然跟着指针走。
**Reset view** 把缩放、平移和线条一并送回起始状态；在没有东西可还原时，这个
按钮是禁用的。
...
换到另一页会从整页重新开始：缩放和平移属于设定它们的那一页，带过去只会落在
下一页碰巧有东西的地方。无论从哪条路换页都一样——页码条、方向键、按钮。
